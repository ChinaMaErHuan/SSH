...
public class TzBeanUtil {
	protected static Logger logger = LoggerFactory.getLogger(TzBeanUtil.class);
	...
	public static Object setDefaultValue(Object bean, Object template) {
		...
		for (Map.Entry<String, Object> entry : defaultMap.entrySet()) {
			if (newMap.get(entry.getKey()) == null) {
				try {
					TzBeanUtil.setFieldValue(bean, entry.getKey(), entry.getValue());
				} catch (NoSuchFieldException e) {
					e.printStackTrace();
				}
			}
